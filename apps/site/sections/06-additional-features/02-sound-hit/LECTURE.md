...
:::code[`create` の中の `pointerup` ハンドラの中（`setVelocity` の後）]{filepath=scenes/game-scene.js offset=148}
...
:::code[`update` の中の `for` の中（スコアを更新した後）]{filepath=scenes/game-scene.js offset=171}
