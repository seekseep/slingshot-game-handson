...
:::code[トップレベル（`GameScene` の後）]{filepath=main.js offset=142}
...
:::code[`GameScene` の `create` の中の `pointerup` ハンドラの中（末尾の `delayedCall` を書き換え）]{filepath=main.js offset=131}
...
:::code[`new Phaser.Game({ ... })` に渡す設定オブジェクトの `scene`]{filepath=main.js offset=176}
