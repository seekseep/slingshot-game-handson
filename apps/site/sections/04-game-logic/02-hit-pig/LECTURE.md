...
:::code[`create` の中、ブタを作る `for` の中（`this.matter.add.gameObject(pig, ...)` の後）]{filepath=scenes/game-scene.js offset=41}
...
:::code[`create` の中、`spawnBird` の中（`bird.setStatic(true)` の後）]{filepath=scenes/game-scene.js offset=96}
...
:::code[`GameScene` クラスの中（`create` の後に、メソッドとして追加）]{filepath=scenes/game-scene.js offset=145}
