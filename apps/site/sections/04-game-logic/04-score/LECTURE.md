...
:::code[`create` の中（`this.pigsLeft` を決めたコードの後）]{filepath=scenes/game-scene.js offset=51}
...
:::code[`update` の中の `for (const pig of this.pendingRemoval)` の中]{filepath=scenes/game-scene.js offset=154}
